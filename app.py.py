...
def check_status_periodically():
    while True:
        with app.app_context():
            ...
            dossiers = conn.execute("SELECT id, lien_suivi_cnaps FROM dossiers").fetchall()
            for dossier in dossiers:
                id_dossier = dossier['id']
                url = dossier['lien_suivi_cnaps']
                if not url:
                    continue
                try:
                    ...
                    if "accepté" in text:
                        ...
                        statut = ""
                    conn.execute("UPDATE dossiers SET check_auto = ? WHERE id = ?", (statut, id_dossier))
                    conn.commit()
                except:
                    continue
            conn.close()
...
def check_status_periodically():
    ...
    while True:
        try:
            ...
            print(f"[Erreur check automatique] {e}")
        time.sleep(300)

# Lancement du thread automatique
threading.Thread(target=check_status_periodically, daemon=True).start()
